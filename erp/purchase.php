...
if (isset($_POST['delete'])) {
    ...
    // $qry = mysqli_query($class->conn,"SELECT purchase_products.* ,invoice.*,jk_user.* , vendor.*
         // FROM purchase_products LEFT JOIN invoice ON purchase_products.`purchase_invoice_id` = invoice.`in_id` 
          // LEFT JOIN jk_user ON purchase_products.purchase_invoice_id = jk_user.`user_id` 
          // LEFT JOIN vendor ON purchase_products.purchase_invoice_id = vendor.`vendor_id`
           // ");
        ...
        <div class="main-content" >
            <div class="wrap-content container" id="container">
                ...
                <section id="page-title">
                    <div class="row">
                        <div class="col-sm-8">
                            <span class="mainDescription">Purchase List</span>
                        </div>
                        <ol class="breadcrumb">
                            <li>
                                <span>Home</span>
                                        ...
                                        <div style="overflow-x:auto;">
                                            <table class="table table-sm table-responsive table-striped table-bordered table-hover table-full-width" id="sample_1">
                                                ...
                                                <?php
                                            		...
                                            		$qry_user = mysqli_query($class->conn,"SELECT purchase_products.* ,invoice.*,jk_user.* , vendor.*
                                                     FROM purchase_products LEFT JOIN invoice ON purchase_products.`purchase_invoice_id` = invoice.`in_id` 
                                                      LEFT JOIN jk_user ON purchase_products.purchase_invoice_id = jk_user.`user_id` 
                                                      LEFT JOIN vendor ON purchase_products.purchase_invoice_id = vendor.`vendor_id`
                                            		  WHERE  purchase_products.added_by ='$user_id';
                                                       ");
                                                while($user_res = mysqli_fetch_array($qry_user))
                                                {	@extract($user_res);
                                                    ?>
                                                    <tr>
